export const filterMenuRecipes = (inventory, ingredients, recipes) => {
  return recipes.filter(recipe => {
    const inventoryLowerCase = inventory.map(ingredient => ingredient.toLowerCase());
    ...
    const ingredientsOptions = ingredients.map(ingredient => ingredient.strIngredient1.toLowerCase());
